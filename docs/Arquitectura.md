...
- `herido.gd` suma `enum EstadoSalud {ESTABLE, CRITICO, AGONIZANTE, MUERTO, ESTABILIZADO}` con decaimiento por **medidor de vitalidad 0..100** (`@export vitalidad_maxima/segundos_por_punto/umbral_critico/umbral_agonizante/factor_gravedad`, RNF-12; reemplaza en S11 a los tres temporizadores `duracion_estable/critico/agonizante` de 60/45/30 s, que dejaban demasiado poco margen para tratar 2-3 heridas). El estado semafórico es ahora una lectura de la vitalidad y no una FSM con reloj propio: tratar puede devolver puntos y hacerlo **mejorar** de AGONIZANTE a CRITICO independiente de `SecuenciaTratamiento`: el jugador debe completar el tratamiento **antes** de que el temporizador llegue a `MUERTO`, que es la tensión de triaje del pilar 4 de `Contexto.md §2`. `MUERTO` y `ESTABILIZADO` son terminales: `aplicar_tratamiento()` devuelve `false` en ambos (RF-14, ya no rescatable).
