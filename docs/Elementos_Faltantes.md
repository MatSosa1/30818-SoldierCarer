...
|---|---|---|---|---|---|---|---|---|
| PH-001 | Manos + uniforme del jugador VR | Modelo 3D + animación | Rig del jugador (`views/Jugador.tscn`) | Rig sin manos (demo no-VR rota con mouse) | Existe `assets/3D/entities/medic/Medic.blend` + brazos (`MilitaryLeftArm.png`/`MilitaryRightArm.png`). Falta: rig VR + animaciones de gesto (vendaje, sutura, disparo) e integración al `XROrigin3D` | S1 | — | En producción |
...
| PH-006 | Pistola del jugador | Modelo 3D | `Pistola.tscn` | Por definir (primitiva) | Existe `assets/3D/weapons/pistol/pistol.blend`. Falta: funda de cadera + integración con la lógica de disparo/recarga | S5 | — | En producción |
