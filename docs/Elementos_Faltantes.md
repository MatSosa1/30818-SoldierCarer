...
| PH-014 | Mapa 2D de muñeca (arte) | UI/textura | `views/MapaMuneca.tscn`, instanciado en `Jugador.tscn` bajo `ManoIzquierda` | **[S2]** Mecánica completa y funcional (RF-06..RF-10): `QuadMesh` azul eléctrico como fondo, icono esfera para el jugador (centro fijo), iconos dinámicos por herido (`SphereMesh`, gris = urgencia real pendiente de S4 / verde = estabilizado), 2 iconos fijos `E1`/`E2` con `Label3D` para saltar de escenario. Activación al levantar la mano izq. por encima de un umbral de altura; selección por proximidad de la mano derecha + gatillo | Existe `assets/2D/2D_neon_map` (`map.blend` + `final.png`). Falta: reemplazar `Fondo`/iconos primitivos por el arte holográfico final e iconos de herido definitivos, sin tocar `mapa_muneca.gd` | S2/S9 | — | En producción |
